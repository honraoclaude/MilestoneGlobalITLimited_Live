...
export default function AccountantIntakePage() {
  ...
  async function sendToAgent(userText: string, history: Msg[]) {
    setS('thinking')
    try {
      const res = await fetch('/api/accountants/intake', {
        method: 'POST',
    ...
    <main className="min-h-screen bg-[#0a0a0f] px-4 py-10 flex items-start justify-center">
      <div className="w-full max-w-md">
        ...
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-xs mt-1">Emma — AI receptionist for an accounting firm</p>
        </div>

        <div className="glass-card p-8">
          <div className="flex flex-col items-center gap-5 mb-6">
            <div className="relative flex items-center justify-center">
              {status === 'speaking' && <div className="absolute w-32 h-32 rounded-full bg-[#10b981]/10 animate-ping" />}
              {status === 'listening' && <div className="absolute w-28 h-28 rounded-full border-2 border-[#10b981]/40 animate-pulse" />}
              <div className="relative w-20 h-20 rounded-full bg-gradient-to-br from-[#10b981] to-[#8b5cf6] flex items-center justify-center text-3xl shadow-lg shadow-[#10b981]/20">
                👩‍💼
              </div>
            </div>
            <div className="text-center">
              <div className="text-white font-bold">Emma</div>
              <div className="text-slate-400 text-sm">AI Receptionist · Clarke & Associates</div>
            ...
            <div ref={scrollRef} className="max-h-56 overflow-y-auto space-y-2 mb-6 p-3 rounded-xl bg-black/20 border border-white/5">
              {msgs.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] text-xs px-3 py-2 rounded-xl leading-relaxed ${m.role === 'user' ? 'bg-[#10b981]/15 text-[#10b981] rounded-tr-sm' : 'bg-white/5 text-slate-300 rounded-tl-sm'}`}>
                    <span className="font-semibold opacity-60 mr-1.5">{m.role === 'user' ? 'You' : 'Emma'}</span>{m.content}
                  </div>
                </div>
              ))}
              {status === 'thinking' && (
          ...
          <div className="flex flex-col items-center gap-4">
            {status === 'idle' && (
              <>
                ...
                <p className="text-slate-600 text-xs text-center max-w-[240px]">Call Emma to experience how AI handles client intake — try asking about self-assessment, VAT, or year-end accounts.</p>
              </>
            )}
            {isActive && (
              <div className="flex items-center gap-10">
                <div className={`flex flex-col items-center gap-1 transition-opacity ${status === 'listening' ? 'opacity-100' : 'opacity-30'}`}>
                  <div className={`w-12 h-12 rounded-full bg-[#10b981]/10 border border-[#10b981]/30 flex items-center justify-center ${status === 'listening' ? 'animate-pulse' : ''}`}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="9" y="2" width="6" height="12" rx="3" fill="#10b981"/><path d="M5 10a7 7 0 0 0 14 0" stroke="#10b981" strokeWidth="1.5" strokeLinecap="round"/><line x1="12" y1="17" x2="12" y2="21" stroke="#10b981" strokeWidth="1.5" strokeLinecap="round"/><line x1="9" y1="21" x2="15" y2="21" stroke="#10b981" strokeWidth="1.5" strokeLinecap="round"/></svg>
                  </div>
                  <span className="text-[#10b981] text-xs">{status === 'listening' ? 'Speak now' : status === 'thinking' ? 'Thinking' : 'Speaking'}</span>
                </div>
            ...
            {status === 'ended' && (
              <div className="flex flex-col items-center gap-3">
                ...
                <button onClick={startCall} className="px-5 py-2.5 rounded-xl bg-[#10b981]/10 text-[#10b981] text-sm font-semibold border border-[#10b981]/20 hover:bg-[#10b981]/20 transition-colors">Call Again</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 glass-card p-4">
          ...
          <p className="text-slate-400 text-xs">Receptionist handles 30–40 intake calls/week @ 6 mins each = <span className="text-white">3–4 hours saved</span>. Never misses an out-of-hours enquiry.</p>
        </div>
      </div>
    </main>
  )

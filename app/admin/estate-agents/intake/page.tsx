...
export default function EstateAgentIntakePage() {
  ...
  async function sendToAgent(userText: string, history: Msg[]) {
    setS('thinking')
    try {
      const res = await fetch('/api/estate-agents/intake', {
        method: 'POST',
    ...
    <main className="min-h-screen bg-[#0a0a0f] px-4 py-10 flex items-start justify-center">
      <div className="w-full max-w-md">
        ...
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-xs mt-1">James — AI receptionist for an estate agency</p>
        </div>

        <div className="glass-card p-8">
          <div className="flex flex-col items-center gap-5 mb-6">
            <div className="relative flex items-center justify-center">
              {status === 'speaking' && <div className="absolute w-32 h-32 rounded-full bg-[#f59e0b]/10 animate-ping" />}
              {status === 'listening' && <div className="absolute w-28 h-28 rounded-full border-2 border-[#f59e0b]/40 animate-pulse" />}
              <div className="relative w-20 h-20 rounded-full bg-gradient-to-br from-[#f59e0b] to-[#f472b6] flex items-center justify-center text-3xl shadow-lg shadow-[#f59e0b]/20">
                🧑‍💼
              </div>
            </div>
            <div className="text-center">
              <div className="text-white font-bold">James</div>
              <div className="text-slate-400 text-sm">AI Receptionist · Hartley & Co</div>
            ...
            <div ref={scrollRef} className="max-h-56 overflow-y-auto space-y-2 mb-6 p-3 rounded-xl bg-black/20 border border-white/5">
              {msgs.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] text-xs px-3 py-2 rounded-xl leading-relaxed ${m.role === 'user' ? 'bg-[#f59e0b]/15 text-[#f59e0b] rounded-tr-sm' : 'bg-white/5 text-slate-300 rounded-tl-sm'}`}>
                    <span className="font-semibold opacity-60 mr-1.5">{m.role === 'user' ? 'You' : 'James'}</span>{m.content}
                  </div>
                </div>
              ))}
              {status === 'thinking' && (
          ...
          <div className="flex flex-col items-center gap-4">
            {status === 'idle' && (
              <>
                ...
                <p className="text-slate-600 text-xs text-center max-w-[240px]">Call James to experience how AI handles estate agent enquiries — try asking about a property, booking a viewing, or a valuation.</p>
              </>
            )}
            {isActive && (
              <div className="flex items-center gap-10">
                <div className={`flex flex-col items-center gap-1 transition-opacity ${status === 'listening' ? 'opacity-100' : 'opacity-30'}`}>
                  <div className={`w-12 h-12 rounded-full bg-[#f59e0b]/10 border border-[#f59e0b]/30 flex items-center justify-center ${status === 'listening' ? 'animate-pulse' : ''}`}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="9" y="2" width="6" height="12" rx="3" fill="#f59e0b"/><path d="M5 10a7 7 0 0 0 14 0" stroke="#f59e0b" strokeWidth="1.5" strokeLinecap="round"/><line x1="12" y1="17" x2="12" y2="21" stroke="#f59e0b" strokeWidth="1.5" strokeLinecap="round"/><line x1="9" y1="21" x2="15" y2="21" stroke="#f59e0b" strokeWidth="1.5" strokeLinecap="round"/></svg>
                  </div>
                  <span className="text-[#f59e0b] text-xs">{status === 'listening' ? 'Speak now' : status === 'thinking' ? 'Thinking' : 'Speaking'}</span>
                </div>
            ...
            {status === 'ended' && (
              <div className="flex flex-col items-center gap-3">
                ...
                <button onClick={startCall} className="px-5 py-2.5 rounded-xl bg-[#f59e0b]/10 text-[#f59e0b] text-sm font-semibold border border-[#f59e0b]/20 hover:bg-[#f59e0b]/20 transition-colors">Call Again</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 glass-card p-4">
          ...
          <p className="text-slate-400 text-xs">Handles 30–50 inbound enquiry calls/week @ 5 mins each = <span className="text-white">3–4 hours saved</span>. Captures out-of-hours leads that would otherwise be lost.</p>
        </div>
      </div>
    </main>
  )

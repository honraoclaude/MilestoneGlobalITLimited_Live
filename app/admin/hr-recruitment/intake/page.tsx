...
export default function HRIntakePage() {
  ...
  async function sendToAgent(userText: string, history: Msg[]) {
    setS('thinking')
    try {
      const res = await fetch('/api/hr-recruitment/intake', {
        method: 'POST',
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0f] px-4 py-10 flex items-start justify-center">
      <div className="w-full max-w-md">
        ...
        <div className="mb-6">
          <h1 className="text-white font-black text-xl">Candidate Intake Voice Agent</h1>
          <p className="text-slate-500 text-xs mt-1">Lauren — AI recruiter for a recruitment agency</p>
        </div>

        <div className="glass-card p-8">
          <div className="flex flex-col items-center gap-5 mb-6">
            <div className="relative flex items-center justify-center">
              {status === 'speaking' && <div className="absolute w-32 h-32 rounded-full bg-[#6366f1]/10 animate-ping" />}
              {status === 'listening' && <div className="absolute w-28 h-28 rounded-full border-2 border-[#6366f1]/40 animate-pulse" />}
              <div className="relative w-20 h-20 rounded-full bg-gradient-to-br from-[#6366f1] to-[#10b981] flex items-center justify-center text-3xl shadow-lg shadow-[#6366f1]/20">
                👩‍💼
              </div>
            </div>
            <div className="text-center">
              <div className="text-white font-bold">Lauren</div>
              <div className="text-slate-400 text-sm">AI Recruiter · Sterling Recruitment</div>
            ...
            <div ref={scrollRef} className="max-h-56 overflow-y-auto space-y-2 mb-6 p-3 rounded-xl bg-black/20 border border-white/5">
              {msgs.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] text-xs px-3 py-2 rounded-xl leading-relaxed ${m.role === 'user' ? 'bg-[#6366f1]/15 text-[#6366f1] rounded-tr-sm' : 'bg-white/5 text-slate-300 rounded-tl-sm'}`}>
                    <span className="font-semibold opacity-60 mr-1.5">{m.role === 'user' ? 'You' : 'Lauren'}</span>{m.content}
                  </div>
                </div>
              ))}
              {status === 'thinking' && (
          ...
          <div className="flex flex-col items-center gap-4">
            {status === 'idle' && (
              <>
                ...
                <p className="text-slate-600 text-xs text-center max-w-[240px]">Call Lauren to experience how AI handles candidate intake — try registering as a candidate looking for a software engineering role.</p>
              </>
            )}
            {isActive && (
              <div className="flex items-center gap-10">
                <div className={`flex flex-col items-center gap-1 transition-opacity ${status === 'listening' ? 'opacity-100' : 'opacity-30'}`}>
                  <div className={`w-12 h-12 rounded-full bg-[#6366f1]/10 border border-[#6366f1]/30 flex items-center justify-center ${status === 'listening' ? 'animate-pulse' : ''}`}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="9" y="2" width="6" height="12" rx="3" fill="#6366f1"/><path d="M5 10a7 7 0 0 0 14 0" stroke="#6366f1" strokeWidth="1.5" strokeLinecap="round"/><line x1="12" y1="17" x2="12" y2="21" stroke="#6366f1" strokeWidth="1.5" strokeLinecap="round"/><line x1="9" y1="21" x2="15" y2="21" stroke="#6366f1" strokeWidth="1.5" strokeLinecap="round"/></svg>
                  </div>
                  <span className="text-[#6366f1] text-xs">{status === 'listening' ? 'Speak now' : status === 'thinking' ? 'Thinking' : 'Speaking'}</span>
                </div>
            ...
            {status === 'ended' && (
              <div className="flex flex-col items-center gap-3">
                ...
                <button onClick={startCall} className="px-5 py-2.5 rounded-xl bg-[#6366f1]/10 text-[#6366f1] text-sm font-semibold border border-[#6366f1]/20 hover:bg-[#6366f1]/20 transition-colors">Call Again</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 glass-card p-4">
          ...
          <p className="text-slate-400 text-xs">25 mins per registration × 20 candidates/week = <span className="text-white">8+ hours saved</span>. Candidates registered instantly, even out of hours.</p>
        </div>
      </div>
    </main>
  )
